...
/**
 * Lo que comparten los cinco rieles de reserva del Bloque B.
 *
 * Vivia dentro de `reservas.spec.ts`, que habia llegado a 2.226 lineas con los
 * cinco flujos, sus dos armadores y el verificador del BackOffice adentro. Se
 * partio el 2026-09-05 en un archivo por riel —clasico, circuitos y series— y
 * esto es lo comun a los tres.
 *
 * **No es un archivo de test**: no termina en `.spec.ts`, asi que Playwright no
 * lo levanta como suite.
 *
 * El tramo del BackOffice es identico se haya reservado un servicio, un hotel,
 * una oferta, un multidestino o una serie: cambia lo que se reservo, no lo que
 * hay que verificar despues.
 */

export const SALTO = String.fromCharCode(10);

/** Fecha sin ceros a la izquierda: el BO la arma como dia/mes/anio pelado. */
export const sinCeros = (fecha: string) => fecha.split('/').map((p) => String(Number(p))).join('/');
 ...
 */
export const importeDelPortal = (texto: string): { moneda: string; valor: number | null } => {
  const limpio = (texto || '').replace(/\s+/g, ' ').trim();
  const moneda = limpio.match(/[A-Z]{3}/)?.[0] ?? '';
 ...
 */
export async function completarCheckoutYEmitir(opciones: {
  page: Page;
  carrito: CarritoPage;
  reserva: {
 ...
 */
export async function verificarEnElBackOffice(opciones: {
  page: Page;
  bo: BackOfficePage;
  codigo: string;
  reserva: {
 ...
 */
export async function armarCircuitoYEmitir(opciones: {
