...
- **Caja diaria falló en la corrida completa #79 por el test, no por el BO (corregido el 2026-09-22).** Se trabó al abrir "Gastos & Mov." desde el menú: la sección ya estaba abierta, el test le hizo clic igual, la cerró y el link quedó animándose ("not stable") 30 segundos. Alan verificó a mano que el menú abre bien. Las cinco pantallas del Bloque C que navegan por el menú (caja diaria, facturas de cliente y de proveedor, órdenes de cobro y de pago) pasaron a `clicEnMenuDelBO` en `utils/pasos.ts`, que abre la sección solo si no tiene la clase `open`. Bloque C completo en local: 11 de 11 en verde.
