...
- **Tablero publicado y andando desde la corrida 70:** https://alan2527.github.io/QA-Climbs/tablero/ . Cada corrida publica `tablero/corridas.json` (índice de las últimas 30) y `tablero/corridas/{n}.json` (detalle prueba por prueba), con `tools/resumen-corrida.mjs` leyendo el reporter `json` de Playwright. La página es `tablero/index.html` (más `favicon.svg`), con los valores de las hojas del WebAdmin y el acento de la agencia con la que se entra al admin en QA (`#6b72f2`), marca Climbs (logo redibujado en SVG a partir de la captura de Alan). Resumen muestra todo; Tarifario, Reservas, Cobranzas e Historial muestran solo su tabla (`#tarifario`, etc.). Cada prueba muestra su duración y cada área el total. **El tablero no consulta Azure DevOps**: el hallazgo de una falla sale solo del campo `defecto` de `tablero/pruebas.json`, cargado a mano (cuando se corrige, sacarlo; el tablero avisa "Pasó: revisar si el Hallazgo N ya se corrigió"). Lo que sí calcula solo, con las corridas publicadas: si la prueba "dejó de funcionar en #N" (pasaba en la anterior) o "falla desde #N". Por eso se sacó "Falla nueva". Lo publicado antes (reportes de Allure y tablero) se trae a `./gh-pages` antes de generar el reporte; el tablero lee de `gh-pages/tablero`. El resumen se arma en `tablero-publicar/` y se copia con `sudo`, porque `allure-history` queda a nombre de root (así falló el paso en la corrida 69).
- **El botón "Correr pruebas" abre la pantalla del workflow en GitHub.** Correr desde el tablero necesita el Worker del monitoreo como intermediario del token: etapa siguiente.
- **Línea base de Excursiones, Cena Show y Ofertas recapturada el 2026-09-21** (corridas 68 a 72 en rojo). No era la agencia: cada login limpia la suplantación (`WebUserManager.cs:271`). Excursiones y Cena Show fallaban solo por mayúsculas: hasta el deploy del 20/09 (PR 6588) el portal mostraba la primera palabra del tipo de tarifa y del PAX en mayúscula ("MENORES en regular", "CENA Show VIP", "MÍNIMO 2 pasajeros") y ahora no; en el WebAdmin los datos están en minúscula. No se encontró la línea que lo hacía. Ofertas: venció la vigencia hasta el 19/09 y el portal pasó a la que termina el 30/09. Verificado: los tres en verde contra la línea base nueva. El 22/09 Paquetes pasó a usar la misma alineación de vigencias que Ofertas (fallaba por la temporada vencida el 19/09).
- **Hallazgo 11 corregido** (verificado el 2026-09-22): el explorador de Paquetes ya muestra el nombre completo, y el test pasa ese paso contra el nombre fijo de `candidatos.json`.
- **"Hoteles, paquetes y ofertas en otro idioma", adaptado el 2026-09-22.** En Paquetes la card recorta "(6 días / 5 noches)" del nombre y muestra las noches en su propio tag (`NewTourTariffControl.ascx.cs`, `StripDuration`); la oferta no lo recorta. El test compara el nombre sin la duración solo en Paquetes y, a pedido de Alan, valida que el tag diga las noches en el idioma elegido ("5 noches" / "5 nights" / "5 noites").
- **Hallazgo nuevo (2026-09-22, bug redactado por Alan el mismo día): el tag de noches de Paquetes no se traduce.** En inglés y en portugués dice "5 noches". Causa verificada: el recurso `Advisor.Tariff.Card.Nights` (y `NightOne`) lo agregó el rediseño 4763 (`c9f4074b`, 15/09) solo en el código, con el texto por defecto "{0} noches"; en `qa.dbo.LocaleStringResource` no existe en ningún idioma y no hay script que lo cree.
- **Caja diaria falló en la corrida completa #79 por el test, no por el BO (corregido el 2026-09-22).** Se trabó al abrir "Gastos & Mov." desde el menú: la sección ya estaba abierta, el test le hizo clic igual, la cerró y el link quedó animándose ("not stable") 30 segundos. Alan verificó a mano que el menú abre bien. Las cinco pantallas del Bloque C que navegan por el menú (caja diaria, facturas de cliente y de proveedor, órdenes de cobro y de pago) pasaron a `clicEnMenuDelBO` en `utils/pasos.ts`. **El criterio no es la clase `open`**: con ella, en la corrida 83 falló "Validaciones: el BO rechaza lo que no debe permitir" porque la sección estaba plegada sin esa clase y el clic lo interceptó su `li`. El BO mismo se guía por la **altura del submenú** (`main.js:357`), y eso es lo que mira ahora, esperando además a que termine de desplegarse. Bloque C completo en local: 11 de 11 en verde.
- **Hallazgo nuevo (2026-09-22, bug redactado por Alan el mismo día): el detalle del multidestino muestra el título del paquete en español** con el sitio en inglés (`customtours/detail.aspx`); en el armado (`customtours/main.aspx`) sale bien y los servicios del detalle sí se traducen. Lo marca la prueba "Multidestino en otro idioma", que ya no corta en el armado. Verificado a mano por Alan.
- **"Contenido en inglés y portugués"** falló una sola vez en la #76 (nombre en inglés de Tigre y Delta vacío) y pasó en local: esporádica, a vigilar.
- **Ofertas ya no depende de la fecha (2026-09-21).** La oferta 5060 vale hasta 2031, pero su tabla viene en 17 temporadas con precio propio, y cada vez que terminaba una cambiaba la primera fila. Alan descartó alargar la vigencia en QA. Ahora el test lleva la línea base y la pantalla al mismo tramo (de hoy al día de la captura + 18 meses), saca las temporadas vencidas y las que aparecen al final, y compara los precios tal cual (`alinearVigencias` en `utils/pasos.ts`, activado con `alinearVigencias: true` en `candidatos.json`, solo en Ofertas). Probado simulando el 01/10, mediados de diciembre y marzo de 2027, más un precio cambiado que tiene que fallar, y el test real en verde. Pasados 18 meses desde la captura pide recapturar.
- **Monitoreo de producción: desplegado y andando desde el 2026-09-21.** Cloudflare Worker `amv-monitoreo` en https://amv-monitoreo.climbs-qa.workers.dev (estado en `/estado`), cada 5 minutos, aviso a Teams con 2 fallas seguidas, repetición cada 30 min, aviso de vuelta con la duración y resumen diario a las 9. Portal (`/login.aspx`), WebAdmin y BO se validan por el campo `txtPassword` del login. **El portal no se chequea en `/online/`**: en producción redirige a `/home/`, una app Next.js que arma el login en el navegador, y eso dio una falsa alarma el mismo 21/09. **API activa desde el 2026-09-22**: `https://api.amv.travel/location/getlanguages` (Whole.Api, solo lectura, lee de la base; 200 con 4 idiomas en QA y en producción). Falta que Nico confirme que `api.amv.travel` es la dirección que usa el portal en producción (`Web.Api.BaseAddress` es un setting de la base).
  - Cuenta de Cloudflare de Alan (alan.herrera@amv.travel), subdominio `climbs-qa.workers.dev`, sin otros miembros por ahora. Espacio KV `ESTADO` (id en `wrangler.toml`). Secretos `TEAMS_WEBHOOK` y `CLAVE` cargados por Alan; no están en el repo.
  - Avisos en el canal privado de Teams **QA Climbs – Alertas**, por el flujo "Enviar alertas de webhook a un canal". Probado: llegó la tarjeta de prueba.
  - **Desplegar lo corre Alan** en su PowerShell (`npx wrangler deploy`, antes `$env:NODE_OPTIONS="--dns-result-order=ipv4first"`): la sesión de Wrangler no se puede usar desde la terminal de Claude, y su red cortó una vez el login por IPv6.
  - **Todos los sitios, por cliente (2026-09-22):** con la lista de Nico, el monitor vigila 18 direcciones: AMV (portal, WebAdmin, BackOffice, API y API de IA en `api.net.amv.travel/health`), Travel Solutions, Kinich y GPS Travel (portal en `/login.aspx`, Sherpa y API en `location/getlanguages`), la portada nueva de AMV y los sitios públicos climbs.dev, micdmc.com y six.travel. `api.climbs.dev` queda declarado pero inactivo: todavía no se usa y hoy no levanta (500.30). Un cliente nuevo es una línea en `CLIENTES` (`src/sitios.js`). Los cuerpos se leen hasta 64 KB y cortando apenas aparece el marcador, para no pasar los 10 ms de procesador del plan gratis. Verificado contra producción: los 18 en OK.
  - Pendiente: que al mismo canal lleguen también los resultados de las corridas de la suite.
  - **Último punto, a pedido de Nico (2026-09-22): pruebas logueadas en producción** (la capa 4: un navegador que entra con un usuario de prueba y recorre, por ejemplo, el tarifario). Depende de los usuarios de prueba que está analizando Nico; el proceso se define con él. Hasta entonces el monitor no valida que el tarifario traiga datos.
  - **Lo que definió Nico el 2026-09-21** sobre la propuesta por capas: la capa 1 (disponibilidad) es la clave, y hay que sumar **todos los sitios que tienen**, no solo AMV (se le pidió la lista con las URL; en el código aparecen Bytour, Avoris, MICDMC y experiences.amv.travel). La capa 2 (las 47 llamadas de catálogo de solo lectura de la API) va; se le propuso cada 15 minutos. La capa 3 (errores del log) queda para después de depurar el log, que hoy tiene mucho ruido. La infraestructura (CPU, memoria) ya la cubre Nico con aviso por mail; más adelante, que avise también por Teams.
  - Inventario de la API al 2026-09-21: publica 235 llamadas y el WEB usa 94 (47 de catálogo, 22 que leen reservas o cotizaciones de un usuario, 6 que van a proveedores externos y 19 que escriben datos). Las que escriben no se pueden llamar desde el monitor: crearían reservas reales.
- **Historial de Allure corregido el 2026-09-22** (a pedido de Alan): el workflow no traía `gh-pages` a `./gh-pages`, así que cada corrida publicaba solo su reporte y se perdía la tendencia; y le pasaba `keep_histories`, que la acción no conoce (el nombre es `keep_reports`). Ahora se trae lo publicado y se conservan los últimos 5 reportes. A vigilar el peso de la publicación: con 5 reportes de los bloques B y C, que tienen videos, sube bastante.
...
**Verificado en la base el 2026-09-16: la tarifa está en pesos**, como planteó Alan.
El servicio 5 tiene `CurrencyID` 8 (**ARS**), `Markup` 1,00 y `NoResidentChangeRate`
1,00, y una sola tarifa Regular publicada (`ServiceRate` 272873): **ARS 47.300 por
pasajero**, de 1 a 1 pax, vigente hasta 2031.

Con eso se ve dónde nacen los decimales:

1. En pesos no hay diferencia: la ficha redondea 47.300 × 2 = 94.600 y el carrito
   47.300, los dos enteros.
2. Los decimales aparecen **al convertir a dólares y aplicar los markups**.
3. El precio por pasajero se acota con lo que guardó el carrito, sin conocer la
   cotización: `TotalRate` 19 = RoundUp(precio × 2), así que cada pasajero vale **más de
   USD 9,00 y hasta USD 9,50**.
4. Con cualquier valor de ese rango la ficha da 10 × 2 = **20** y el carrito **19**.

La tarifa en pesos es lo que produce los decimales; lo que hace que ficha y carrito no
coincidan es que **redondean en distinto momento** desde la US 4739.
...
### US 4735 — tags de servicio en la card del tarifario: hecho el 2026-09-16

**Hecho y en verde.** Alcance acordado con Alan: que los servicios propios tengan tags
y que el test del tarifario **valide su presencia en la card**. Nada más.

La US 4735 (Sprint 23) muestra en el tarifario online los tags asignados a los
servicios, en las solapas Excursión, Traslado y Cena Show. El test cubre el criterio:

> "Dado un servicio con uno o más tags asignados, cuando se muestra su tarjeta en el
> tarifario, entonces deben visualizarse todos los tags asociados a ese servicio en el
> pie de la tarjeta."

**Qué exige.** En `validarCard` (`tests/bloque-a/tarifario.spec.ts`), la card tiene que
mostrar en el pie exactamente los tags de la base: todos, y sin que sobre ninguno. Se
compara como conjunto, no por orden: el orden alfabético sale del código, no de la US.
Los tags se leen de `div.tariff-service-tags span.tariff-service-tag`
(`ServiceTariffControl.ascx.cs:206`) con `TarifarioPage.tagsDeServicio()`, y los
esperados viven en `card.tagsDeServicio` de `data/candidatos.json`.

El texto que muestra la card es `TagDetail.Detail` del idioma del sitio, con fallback a
`Tag.Name` (`TagManager.GetServiceTagsBulk`, repo API). Los esperados son los de
`LanguageID = 1`.

**Datos AUTO-QA creados el 2026-09-15** (**no se borran**, no confundir con los QA4735 8,
9 y 10 de la ejecución manual, que sí):

| Tag | Nombre | Español / Inglés / Portugués |
|---|---|---|
| 11 | AUTO-QA NO TOCAR - Nocturno | AUTO-QA Nocturno / AUTO-QA Night / AUTO-QA Noturno |
| 12 | AUTO-QA NO TOCAR - Gastronomia | AUTO-QA Gastronomía / AUTO-QA Gastronomy / AUTO-QA Gastronomia |

| Servicio | Pestaña | Tags | ServiceTag |
|---|---|---|---|
| 5 Tigre y Delta | Excursiones | Gastronomía y Nocturno | 1029, 1030 |
| 1223 Aeropuerto / Hotel | Traslados | Nocturno | 1031 |
| 163 Café de los Angelitos | Cena Show | Gastronomía | 1032 |

**Verificado el 2026-09-15** corriendo los tres tests: en verde, con los tags leídos del
adjunto de cada card. Los tags del pie no afectaron el resto de la card: descripción,
componentes, observaciones, tooltips e importes siguieron en verde.

**Afuera por decisión**, aunque la US los pide: pills, filtro por tags, sugerencias del
buscador, contador y combinación con proveedor o nombre. Si alguna vez se suman, ya está
relevado: las pills viven en `#tariffCategoryTags` como `button.tariff-category-pill`
(`TariffFilterControl.ascx:401`), el filtro de proveedor usa `BO_ServiceSupplier` y
EXPEDICIONES (338) tiene la 5 como única excursión con tags AUTO-QA. Para probar el "al
menos uno" haría falta un segundo servicio propio en la misma solapa con otro tag.
