...
- **Pendiente de decisión — "Hoteles, paquetes y ofertas en otro idioma"**: la card ya no muestra "(6 días / 5 noches)" en el nombre; el rediseño 4763 lo recorta a propósito porque la duración va en su propio tag (`TourAllCardsControl.ascx.cs:341`). El test espera el nombre completo. Alan decide si se adapta el test o se consulta al PM.
...
- **Allure, de paso:** el workflow le pasa `keep_histories`, pero la acción solo conoce `keep_reports` (lo avisa como warning en cada corrida), así que el 5 no se aplica.
