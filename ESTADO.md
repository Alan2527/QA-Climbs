...
- **Hallazgo nuevo (2026-09-22, sin redactar): el tag de noches de Paquetes no se traduce.** En inglés y en portugués dice "5 noches". Causa verificada: el recurso `Advisor.Tariff.Card.Nights` (y `NightOne`) lo agregó el rediseño 4763 (`c9f4074b`, 15/09) solo en el código, con el texto por defecto "{0} noches"; en `qa.dbo.LocaleStringResource` no existe en ningún idioma y no hay script que lo cree. **Falta la cita de la US** para redactarlo como bug; si no aparece, va como consulta.
...
- **Hallazgo nuevo (2026-09-22, sin redactar): el detalle del multidestino muestra el título del paquete en español** con el sitio en inglés (`customtours/detail.aspx`); en el armado (`customtours/main.aspx`) sale bien y los servicios del detalle sí se traducen. Lo marca la prueba "Multidestino en otro idioma", que ya no corta en el armado. Verificado a mano por Alan. Falta la cita de la US (probablemente la 4613).
