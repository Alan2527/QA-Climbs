...
- **Monitoreo: código hecho en `monitoreo/`, sin desplegar.** Cloudflare Worker, cada 5 minutos, aviso a Teams con 2 fallas seguidas, repetición cada 30 min, aviso de vuelta con la duración y resumen diario a las 9. Autorizado vigilar producción (portal, WebAdmin, BO y API). Portal, WebAdmin y BO se validan por el campo `txtPassword` del login, verificado en QA. **La API queda desactivada**: en QA la raíz da 403 y las rutas del código 404; falta que el dev diga qué endpoint liviano usar. La cuenta de Cloudflare la crea Alan con un mail del equipo; los pasos están en `monitoreo/README.md`. `npm test` en `monitoreo/`: 7 en verde.
